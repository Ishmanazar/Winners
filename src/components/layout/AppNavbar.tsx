...
import { Trophy, Flame, Zap, Edit3, Gamepad2 } from 'lucide-react';

export function AppNavbar() {
  const navigate = useNavigate();
  const location = useLocation();
  const { currentPlayer, getPlayerBestScore } = useGameStore();
  const [isNameModalOpen, setIsNameModalOpen] = useState(false);

  const playerName = currentPlayer.name?.trim();
  const bestScore = playerName ? getPlayerBestScore(playerName) : 0;

  const isPlayActive = location.pathname === '/' || location.pathname === '/lobby';
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            {playerName ? (
              <button
                ...
                className="flex items-center gap-1.5 p-1 sm:px-2.5 sm:py-1 rounded-xl bg-[#1b2b34] hover:bg-[#253945] border-2 border-[#2b3e4a] hover:border-[#1cb0f6] text-white transition-all cursor-pointer group"
                title="Edit Athlete Profile"
              >
                <span className="text-base sm:text-lg">{currentPlayer.avatar || '😎'}</span>
                <span className="font-display font-bold text-xs max-w-[80px] truncate hidden sm:inline">
                  {playerName}
                </span>
                <Edit3 size={12} className="text-white/40 group-hover:text-[#1cb0f6] hidden sm:inline" />
              </button>
            ) : (
              <button
                onClick={() => setIsNameModalOpen(true)}
                className="px-2.5 py-1 rounded-xl bg-[#58cc02] hover:bg-[#61e002] text-white text-xs font-display font-black uppercase tracking-wider border-b-2 border-[#46a302] cursor-pointer"
              >
                SIGN IN
              </button>
            )}
